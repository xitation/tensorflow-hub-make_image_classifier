...
if [ "$1" == "" ]; then
  ...
  echo "You can filter the results using grep -v to find images with wrong format to analyse."
  echo "E.G. \"./file_type_list.sh /mnt/disk/folder_full_of_images |grep -v JPEG\" will show any images in the folder that are not JPEG formatted"
  exit 1
fi
